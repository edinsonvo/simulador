...
def parameter_editor(
    model: str, params: EconomyParameters, key_prefix: str | None = None
) -> EconomyParameters:
    ...
    with st.expander("⚙️ Parámetros del modelo", expanded=True):
        ...
        for key in relevant:
            ...
            step = (hi - lo) / 200.0
            value = st.slider(
                label, lo, hi, float(getattr(params, key)), step, key=f"{prefix}_{key}"
            )
            updates[key] = value
...
def shock_selector(model: str) -> Shock | None:
    ...
    chosen = st.selectbox("Choque", ["(sin choque)"] + labels, key="shock_picker")
...
def build_scenario(
    ...
    scenario = Scenario(
        model=model, parameters=params, metadata=metadata, label=f"Escenario {model}"
    )
